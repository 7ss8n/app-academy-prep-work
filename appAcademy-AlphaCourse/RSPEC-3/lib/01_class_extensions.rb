  ...
  def caesar(shift)
    ...
    self.split("").each do |char|
      if char > "Z"
        ...
        shifted_char_ord -= 26 if shifted_char_ord > "z".ord
        new_string += shifted_char_ord.chr
      else
        shifted_char_ord = (char.ord + shift)
        shifted_char_ord -= 26 if shifted_char_ord > "Z".ord
        new_string += shifted_char_ord.chr
...
class Hash
  def difference(other_hash)
    ...
    unique_to_self = self.reject {|k, v| other_hash.has_key?(k)}
    unique_to_other = other_hash.reject {|k, v| self.has_key?(k)}

    unique_to_self.each {|k, v| new_hash[k] = v}
    unique_to_other.each {|k, v| new_hash[k] = v}
